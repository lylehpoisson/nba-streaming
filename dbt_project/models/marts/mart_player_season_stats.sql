...
with
    game_logs as (select * from {{ ref("stg_player_game_logs") }}),
    ...
    aggregated as (
        select
            ...
            round(
                sum(points)::float / nullif(2 * (sum(fga) + 0.44 * sum(fta)), 0), 3
...
select
    *,
    ...
    case when games_played >= 10 then rank() over (order by ppg desc) end as ppg_rank,
    case
        when games_played >= 10 then rank() over (order by true_shooting_pct desc)
    end as ts_pct_rank
from aggregated
order by ppg desc
